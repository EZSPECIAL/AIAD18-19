...
public class RunAgents {
	...
	private static final int parkCSVCoordsXI = 1;
	private static final int parkCSVCoordsYI = 2;
	private static final int parkCSVHourlyCostI = 6;
	private static int[] parkingLotZoning = new int[9];
	private static int[] parkingLotPricing = new int[27];
	...
	private static void createParkingLotAgents(ContainerController container) throws StaleProxyException {
		...
		// Use fixed parameters read from config file
			...
			// Log parking zone and price range for Data Mining
			parksToCSV();
			
			// Create parking lot agents based on config file
	...
	/**
	 * For each park gets its zone and pricing range for outputting to the CSV file for data mining.
	 */
	private static void parksToCSV() {
		
		for(int i = 0; i < lotConfigArgs.size(); i++) {
			Object[] args = lotConfigArgs.get(i);
			
			Point coords = new Point((int) args[parkCSVCoordsXI], (int) args[parkCSVCoordsYI]);
			int hourlyCost = (int) args[parkCSVHourlyCostI];
			
			parkingLotZoning[calcParkZone(coords)]++;
			parkingLotPricing[calcParkPricingRange(coords, hourlyCost)]++;
		}
		
		Logger.getInstance().setParkCSVData(parkingLotZoning, parkingLotPricing);
	}

	/**
	 * Calculates pricing range based on price and coords of given park. Parks are
	 * divided in 5x5 boxes and price is separated in increments of 5 (up to 15);
	 * 
	 * @param coords the park's coords
	 * @param hourlyCost the park's hourly cost
	 * @return the offset to increment the pricing range array on
	 */
	private static int calcParkPricingRange(Point coords, int hourlyCost) {
		
		int zone = (coords.x / 5) + (coords.y / 5 * 3);
		int costRange = hourlyCost / 6;
		int offset = zone * 3 + costRange;
		
		return offset;
	}

	/**
	 * Calculates park zone of a given park. Parks are divided in 5x5 boxes.
	 * 
	 * @param coords the park's coords
	 * @return the offset to increment the number of parks on
	 */
	private static int calcParkZone(Point coords) {
		return (coords.x / 5) + (coords.y / 5 * 3);
	}
